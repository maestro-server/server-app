...
describe('e2e images', function () {
    ...
    describe('read image', function () {
        ...
        it('count my images', function (done) {
            request(mock)
                .get(`/teams/${teams._id}/images/count`)
                .set('Authorization', `JWT ${user.token}`)
                .expect(200)
                .expect(function (res) {
        ...
        it('autocomplete without token', function (done) {
            request(mock)
                .get(`/teams/${teams._id}/images/autocomplete`)
    ...
    describe('patch image', function () {
        it('patch image, changing name', function (done) {
            ...
            request(mock)
                .patch(`/teams/${teams._id}/images/${images[0]._id}`)
                .send(data)
                .set('Authorization', `JWT ${user.token}`)
                .expect(202)
                .expect('Content-Type', /json/)
    ...
    describe('update image', function () {
        it('put image with valid data', function (done) {
            ...
            request(mock)
                .put(`/teams/${teams._id}/images/${images[0]._id}`)
                .send(data)
                .set('Authorization', `JWT ${user.token}`)
                .expect(202)
                .expect('Content-Type', /json/)
    ...
    describe('confirm update image', function () {
        ...
        it('confirm if any of my updates/patchs dont create new image', function (done) {
            request(mock)
                .get(`/teams/${teams._id}/images`)
                .set('Authorization', `JWT ${user.token}`)
                .expect(200)
    ...
    describe('e2e teams: add roles', function () {
        it('valid data to add roles', function (done) {
            ...
            request(mock)
                .post(`/teams/${teams._id}/images/${images[0]._id}/roles`)
                .send(data)
                .set('Authorization', `JWT ${user.token}`)
                .expect(201)
                .expect('Content-Type', /json/)
    ...
    describe('update roles', function () {
        it('Exist roles - update role image', function (done) {
            request(mock)
                .put(`/teams/${teams._id}/images/${images[0]._id}/roles`)
                .send(images[0].roles)
                .set('Authorization', `JWT ${user.token}`)
                .expect(201)
                .expect('Content-Type', /json/)
    ...
    describe('update single roles', function () {
        it('Exist roles - update role image', function (done) {
            request(mock)
                .put(`/teams/${teams._id}/images/${images[0]._id}/roles/${friend._id}`)
    ...
    describe('delete roles', function () {
        it('Exist roles - delete role', function (done) {
            request(mock)
                .delete(`/teams/${teams._id}/images/${images[0]._id}/roles/${friend._id}`)
                .set('Authorization', `JWT ${user.token}`)
                .expect(204)
                .end(function (err) {
                    if (err) return done(err);
    ...
    describe('delete image', function () {
        it('Exist roles - delete my image', function (done) {
            request(mock)
                .delete(`/teams/${teams._id}/images/${images[0]._id}`)
                .set('Authorization', `JWT ${user.token}`)
                .expect(204)
                .end(function (err) {
                    if (err) return done(err);
