...
describe('e2e datacenters', function () {
    ...
    before(function (done) {
        cleaner_db([{tb: 'users'}, {tb: 'datacenters'}, {tb: 'teams'}], () => {
    ...
    describe('read datacenter', function () {
        ...
        it('count my datacenters', function (done) {
            request(mock)
                .get(`/teams/${teams._id}/datacenters/count`)
                .set('Authorization', `JWT ${user.token}`)
                .expect(200)
                .expect(function (res) {
        ...
        it('autocomplete without token', function (done) {
            request(mock)
                .get(`/teams/${teams._id}/datacenters/autocomplete`)
    ...
    describe('patch datacenter', function () {
        it('patch datacenter, changing name', function (done) {
            ...
            request(mock)
                .patch(`/teams/${teams._id}/datacenters/${datacenters[0]._id}`)
                .send(data)
                .set('Authorization', `JWT ${user.token}`)
                .expect(202)
                .expect('Content-Type', /json/)
    ...
    describe('update datacenter', function () {
        it('put datacenter with valid data', function (done) {
            ...
            request(mock)
                .put(`/teams/${teams._id}/datacenters/${datacenters[0]._id}`)
                .send(data)
                .set('Authorization', `JWT ${user.token}`)
                .expect(202)
                .expect('Content-Type', /json/)
    ...
    describe('e2e teams: add roles', function () {
        it('valid data to add roles', function (done) {
            ...
            request(mock)
                .post(`/teams/${teams._id}/datacenters/${datacenters[0]._id}/roles`)
                .send(data)
                .set('Authorization', `JWT ${user.token}`)
                .expect(201)
                .expect('Content-Type', /json/)
    ...
    describe('update roles', function () {
        it('Exist roles - update role datacenter', function (done) {
            request(mock)
                .put(`/teams/${teams._id}/datacenters/${datacenters[0]._id}/roles`)
    ...
    describe('update single roles', function () {
        it('Exist roles - update role datacenter', function (done) {
            request(mock)
                .put(`/teams/${teams._id}/datacenters/${datacenters[0]._id}/roles/${friend._id}`)
    ...
    describe('delete roles', function () {
        it('Exist roles - delete role', function (done) {
            request(mock)
                .delete(`/teams/${teams._id}/datacenters/${datacenters[0]._id}/roles/${friend._id}`)
                .set('Authorization', `JWT ${user.token}`)
                .expect(204)
                .end(function (err) {
                    if (err) return done(err);
    ...
    describe('delete datacenter', function () {
        it('Exist roles - delete my datacenter', function (done) {
            request(mock)
                .delete(`/teams/${teams._id}/datacenters/${datacenters[0]._id}`)
                .set('Authorization', `JWT ${user.token}`)
                .expect(204)
                .end(function (err) {
                    if (err) return done(err);
